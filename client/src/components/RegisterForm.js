...
const RegisterForm = (props) => {
    ...
    return (
        ...
        return (
            <div className='preForm0'>
                <h1 className='tit1'>REGISTRO</h1>
                <div className='preForm'>
                <Form className= "contact" method= "post" onSubmit={handleSubmit}>
                    <label htmlFor="firstName" className="col-sm-2 col-form-label">Nombre</label>
                    <Field id='firstName'type="text" className="form-control2" placeholder="Nombre" name='firstName'/>
                    {errors.firstName && touched.firstName && <p>{errors.firstName}</p>}
        
                    <label htmlFor="lastName" className="col-sm-2 col-form-label">Apellido</label>
                    <Field  id='lastName' type="text" placeholder="Apellido" className="form-control2" name='lastName'/>
                    ...
                    <Field  id='confirmPassword' type="password" placeholder="Confirmar Contraseña" className="form-control2" name='confirmPassword'/>
                    {errors.confirmPassword && touched.confirmPassword && <p>{errors.confirmPassword}</p>}
                    <br></br>
                    <button className='btnReg' type="submit" disabled={Object.values(errors).length > 0}><p className='regreg'>Registrarse</p></button>
                </Form>
                </div>
                </div>
        );
